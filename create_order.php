...
try {
    $client = new ApgClient($cfg);   // $cfg comes from apg_config()

    $handshake = $client->initiateHandshake($orderRef);

    error_log('APG handshake response for ' . $orderRef . ': ' . print_r($handshake, true)); // TEMP DEBUG - remove after testing

    if (!$handshake['success'] || empty($handshake['auth_token'])) {
        throw new Exception($handshake['error'] ?? 'Handshake failed');
    }

    $htmlForm = $client->buildSsoRedirectForm(
        $handshake['auth_token'],
        $orderRef,
        number_format($total, 2, '.', '')
    );

    echo $htmlForm;   // This should redirect to Bank Alfalah
