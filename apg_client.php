...
class ApgClient
{
    ...
    public function __construct(array $fullConfig)
    {
        ...
        $this->cfg['return_url'] = $fullConfig['return_url'] ?? 'https://www.primevortex.co/return.php';
    }
    ...
    {
        $hashFields = [
            'HS_ChannelId'                  => $this->cfg['1001'],
            'HS_MerchantId'                 => $this->cfg['265397'],
            'HS_StoreId'                    => $this->cfg['560844'],
            'HS_ReturnURL'                  => $this->cfg['https://www.primevortex.co/return.php'],
            'HS_MerchantHash'               => $this->cfg['xyEPuAQ1c8c'],
            'HS_MerchantUsername'           => $this->cfg['ulaqah'],
            'HS_MerchantPassword'           => $this->cfg['ic3kz96QoB5vFzk4yqF7CA=='],
    ...
    public function buildSsoRedirectForm(string $authToken, string $orderRef, string $amount): string
    {
        $hashFields = [
            'AuthToken'                     => $authToken,
            'ChannelId'                     => $this->cfg['1001'],
            'Currency'                      => 'PKR',
            'IsBIN'                         => '0',
            'ReturnURL'                     => $this->cfg['https://www.primevortex.co/return.php'],
            'MerchantId'                    => $this->cfg['265397'],
            'StoreId'                       => $this->cfg['560844'],
            'MerchantHash'                  => $this->cfg['xyEPuAQ1c8c'],
            'MerchantUsername'              => $this->cfg['ulaqah'],
            'MerchantPassword'              => $this->cfg['ic3kz96QoB5vFzk4yqF7CA=='],
            'TransactionTypeId'             => '3',
        ...
        $html = '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Redirecting...</title></head><body style="font-family:sans-serif;">';
        $html .= '<p style="text-align:center; padding:20px;">Continuing to Bank Alfalah Secure Payment...</p>';
        ...
        // TEMP DEBUG: visible submit button + field dump, so this step is visible instead of instant
        $html .= '<div style="text-align:center;"><button type="submit">Continue to Payment (debug: click if not redirected)</button></div>';
        $html .= '</form>';
        $html .= '<pre style="max-width:700px;margin:20px auto;background:#eee;padding:10px;white-space:pre-wrap;word-break:break-all;">DEBUG SSO fields: '
            . htmlspecialchars(json_encode($fields, JSON_PRETTY_PRINT))
            . '</pre>';
        $html .= '<script>setTimeout(function(){document.getElementById("sso").submit();}, 6000);</script></body></html>';
