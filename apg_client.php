...
class ApgClient
{
    ...
    {
        // NOTE: Bank Alfalah's own reference code includes an empty
        // "RequestHash=" field INSIDE the string that gets hashed (right
        // after AuthToken, before ChannelId). This looks redundant but it's
        // required — leaving it out produces a hash Bank Alfalah's server
        // won't recognize, causing a silent handshake/SSO rejection.
        $hashFields = [
            'AuthToken'                     => $authToken,
            'RequestHash'                   => '',
        ...
        // buildHash() joins as "key=value&key=value...&" then trims the
        // trailing "&" — for the empty RequestHash field this correctly
        // produces "...AuthToken=xyz&RequestHash=&ChannelId=..." matching
        // the bank's reference string exactly.
        $requestHash = $this->buildHash($hashFields);

        $fieldsWithoutHash = $hashFields;
        unset($fieldsWithoutHash['RequestHash']);
        $fields = array_merge($fieldsWithoutHash, ['RequestHash' => $requestHash]);
