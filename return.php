 ...
 * after Bank Alfalah shows its own success/failure screen. Per the
 * integration guide, Bank Alfalah appends the order id as "O" (e.g.
 * ?O=A10, sometimes alongside TS/RC/RD). We take that order id and call
 * the IPN "Inquire Transaction" endpoint ourselves to get the
...
try {
    $ipnUrl = rtrim($cfg['https://payments.bankalfalah.com/HS/api/IPN/OrderStatus'], '/') . '/' . rawurlencode($cfg['265397']) . '/' . rawurlencode($cfg['store_id']) . '/' . rawurlencode($orderRef);
