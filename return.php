 ...
 * return.php
 * -----------
 * This is the HS_ReturnURL / ReturnURL the customer's browser lands on
 * after Bank Alfalah shows its own success/failure screen. Bank Alfalah
 * appends the order id as "O" (e.g. ?O=A10, sometimes alongside TS/RC/RD).
 * We take that order id and call the IPN "Inquire Transaction" endpoint
 * ourselves to get the authoritative TransactionStatus — never trust the
 * redirect alone.
 *
 * (The handshake step is now done entirely server-side in create_order.php,
 * so this page no longer needs to handle a separate handshake callback.)
 */
...
try {
    ...
    if (is_array($data) && isset($data['TransactionStatus'])) {
        ...
        render_result($status, $orderRef, apg_get_order($orderRef));
    } else {
        render_result('Unknown', $orderRef, $orderData, 'Could not parse the status response from Bank Alfalah. If you were charged, contact support with your order reference.');
    }
} catch (Throwable $e) {
