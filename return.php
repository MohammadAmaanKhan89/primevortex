...
try {
    ...
    $ipnUrl = rtrim($cfg['ipn_url'], '/') . '/' . $cfg['merchant_id'] . '/' . $cfg['store_id'] . '/' . $orderRef;
    
    error_log("Trying to inquire status: " . $ipnUrl);
    ...
    if (is_array($data) && isset($data['TransactionStatus'])) {
        ...
        render_result($status, $orderRef, apg_get_order($orderRef), 'Raw response: ' . $raw);
    } else {
        render_result('Unknown', $orderRef, $orderData, 'Bank Alfalah returned invalid data. Raw: ' . htmlspecialchars($raw));
    }
} catch (Throwable $e) {
    render_result('Unknown', $orderRef, $orderData, 'Could not connect to Bank Alfalah: ' . $e->getMessage());
}